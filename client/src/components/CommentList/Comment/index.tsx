...
interface Props {
    ...
    onDelete?: Function; // if given, a button will be created where `deleteButton` will be the click handler
}
    ...
    return (
        <article className="comment">
          <Link className="author" to={`/u/${props.profileLink}`}>{props.author}</Link>
            ...
            {props.onDelete ?
                <button type="button" className="deleteReply" onClick={props.onDelete as any}>Delete</button> :
                ''}
        </article>
    );
